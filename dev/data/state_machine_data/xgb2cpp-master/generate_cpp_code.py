...
FEAT2IDX = {name: i for i, name in enumerate(FEATURES)}

def get_single_booster_cpp_code(booster_tree, branch_id, class_index, indentation_level=0):
    line = booster_tree[branch_id].strip()
    level = line.split()

    booster_code = ""

    if 'leaf' in line:
        leaf_val = float(line.split('leaf=')[1])
        booster_code += "{0}sum[{1}] += {2};\n".format("  " * indentation_level, class_index, leaf_val)
        return booster_code
    ...
    m = re.search(r'\[([^\]]+)\]', line)
    if not m:
        raise ValueError("Could not find [...] split part in: " + line)
    ...
    if not op:
        raise ValueError("No operator (<,<=,>,>=) found in: " + inside)
    
    left, right = inside.split(op, 1)
    feat_tok = left.strip()
    thr = right.strip()

    if feat_tok.startswith('f') and feat_tok[1:].isdigit():
        feature_index = int(feat_tok[1:])
    else:
        if feat_tok not in FEAT2IDX:
            raise ValueError(f"Unknown feature name '{feat_tok}' in: " + line)
        feature_index = FEAT2IDX[feat_tok]
